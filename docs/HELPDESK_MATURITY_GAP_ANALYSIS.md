...
| Known-good organizational state (Phase 6) | Real, currently entirely absent — today nothing distinguishes "generic best practice" from "this tenant's actual DNS/config policy." Directly relevant to the mandate's own DNS example (never silently replace corporate DNS with a public resolver) — there is no mechanism today that *could* do that (no such remediation skill exists), but there is also no mechanism to declare and check against an org's actual expected configuration once a knowledge-driven remediation planner exists. |
...
| Simulation/dry-run *execution preview* mode (Phase 14, beyond diagnosis) | Diagnosis is unconditionally simulation-only already (§25 of the architecture audit). A dedicated "show me exactly what this specific pending action would do, including the verification and rollback plan, without running it" view does not exist as its own UI/API surface — today an operator infers this from the `pending_approval` `Action`'s stored manifest fields. Small, real, tractable gap. |
