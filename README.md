...
- An omnichannel help-desk foundation: a channel-agnostic Conversation Service (`helpdesktool/conversation.py`), identity resolution that trusts only an already-authenticated channel identity (never chat message text), and an Application Connector Framework (`helpdesktool/connectors/`) for policy-gated, approval-required self-service password reset / account unlock / MFA reset -- proven end to end via the web chat channel against a dev-safe mock connector. Slack/Teams/Google Chat adapters and real (non-mock) application connectors are the next layer to add on top of this same foundation; see `docs/IMPLEMENTATION_PLAN.md`'s Milestone 11.
