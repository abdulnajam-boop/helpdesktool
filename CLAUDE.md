...
Mirrors `linux_agent/`'s exact contract (same `AgentConfig` shape, same `ControlPlaneClient`, same job-envelope validation in `agent.py`'s `WindowsAgent.execute_job`) so the control plane treats both OSes identically. Differences are purely OS-specific implementation, never a weaker safety model:
