...
- `lease_reaper.py` — separate long-running process (`helpdesk-lease-reaper` entry point) recovering `Action` jobs whose agent claim lease expired without a result ever being reported (agent crash/disconnect) — requeues them (bounded by `Settings.lease_reaper_max_attempts`) or escalates to `failed` once exhausted. One of the four `rls_bypass` call sites, and also upserts its own liveness heartbeat every iteration. Without this recovery logic, a crashed agent left a job `claimed` forever with no operator-visible signal — see `docs/IMPLEMENTATION_PLAN.md` Milestone 3.
- `retention_worker.py` — separate long-running process (`helpdesk-retention-worker` entry point, hourly by default) purging expired `heartbeats`/`device_inventory`/`idempotency_records` rows (`Settings.*_retention_days`). The fourth and last `rls_bypass` call site. **Deliberately never touches `audit_events`** — it's hash-chained, so deleting an old row breaks verification of everything after it; see the module docstring for why this is retained indefinitely by design rather than an oversight.
