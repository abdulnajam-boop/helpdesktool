...
- Dashboard and searchable browser pages for Devices, Tickets, Incidents (with an AI-diagnosis panel), Actions, Approvals, AI Help Desk, Conversations, Applications, Skills, Reports, Audit, Integrations, and Settings.
- An omnichannel help-desk foundation: a channel-agnostic Conversation Service (`helpdesktool/conversation.py`), identity resolution that trusts only an already-authenticated channel identity (never chat message text), and an Application Connector Framework (`helpdesktool/connectors/`) for policy-gated, approval-required self-service password reset / account unlock / MFA reset -- proven end to end via the web channel, Slack, Google Chat, and Microsoft Teams (all three real channel adapters; see `helpdesktool/channels/`) against a dev-safe mock connector. A high-risk connector request now also requires step-up verification -- the requester must retrieve a short-lived code through a separately authenticated call before an approver can act on it -- on top of the existing separation-of-duties rule. Real (non-mock) application connectors are the next layer to add on top of this same foundation; see `docs/HELPDESK_MATURITY_GAP_ANALYSIS.md`.
...
- Device-bound job leases, cryptographically signed and versioned job envelopes (Ed25519, agents pin the public key via trust-on-first-use, with a real key-rotation transition window -- see Known limitations), replay protection, claim-token validation, idempotent reporting, verification and rollback outcomes.
...
- Unprivileged Linux and Windows agents, each with inventory collectors and two allowlisted mutating executors -- `service.restart` (fixed `systemctl` argument vectors on Linux, direct Win32 Service Control Manager calls on Windows) and `dns.flush_cache` (`resolvectl` on Linux, the Win32 `DnsFlushResolverCache` API via `ctypes` on Windows) -- never a shell.
...
This section is kept current as of 2026-08-21; see `docs/HELPDESK_MATURITY_GAP_ANALYSIS.md` for the full, per-milestone audit this summary is drawn from (prefer it over `docs/IMPLEMENTATION_PLAN.md`'s older milestones for current-state questions).
...
- **Job-envelope signing key rotation is now implemented (Milestone 27)** -- bumping one config value (`Settings.job_signing_key_version`) derives a genuinely different key from the same seed, and every already-enrolled agent picks up the new version automatically within one heartbeat interval via a trailing trust window, with no manual re-pinning step. A full break-glass rotation invalidating every prior version still requires changing `job_signing_seed` itself.
- **Real (non-mock) application connectors are BLOCKED-EXTERNAL.** The connector framework and mock implementation are real and fully wired; Entra ID/Microsoft 365, Google Workspace, Okta, and GitHub connectors need real per-tenant credentials this environment doesn't have.
- **Microsoft Teams' exact live Bot Framework behavior is unverified.** `helpdesktool/channels/teams.py` implements the documented Bot Framework JWT contract (issuer/audience/JWKS/`serviceUrl` claim), tested against a locally generated keypair -- like Slack and Google Chat before their own real-world use, it has not been exercised against a live Azure Bot Service registration. Outbound Teams replies, like Slack's, are BLOCKED-EXTERNAL (need a real Azure AD client secret).
...
- **The frontend has no route-level automated test coverage** beyond the OIDC/PKCE login-flow logic (`frontend/src/auth/oidc.test.ts`) -- no React Testing Library component tests yet, and no accessibility audit has been done. The design is still functional-but-minimal; a design-system modernization pass has not started.
...
- **No Terraform or other infrastructure-as-code exists yet.** The only deployment story today is `docker compose`, independently verified fresh-from-zero.
- **No container/release signing.** CycloneDX SBOM generation runs in CI; Sigstore/cosign image or release signing does not exist yet.
...
- `service.restart` and `dns.flush_cache` are the only two mutating reference executors, both real on both agents. Adding a genuinely new mutating skill always requires an agent code change -- the skill registry can declare policy metadata for a skill id, but never ships new execution logic, by design.
- **No `LICENSE` file exists** despite `pyproject.toml` declaring `license = "Apache-2.0"` -- needs a real copyright holder name/year decision from the repository owner; not something to invent.
