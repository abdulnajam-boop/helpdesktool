...
def test_tenant_device_telemetry_ticket_and_approval_workflow(client, monkeypatch):
    ...
    device_detail = http.get(
        f"/v1/devices/{device['device_id']}", headers=browser_headers
    ).json()
    assert device_detail["latest_heartbeat"]["status"] == {"load": 0.2}
    assert device_detail["incidents"][0]["id"] == incidents[0]["id"]
    assert device_detail["audit"]

    recovered = http.post(
        "/v1/development/simulations/low-disk",
    ...
    assert recovered.json()["incident"]["status"] == "resolved"
    incident_detail = http.get(
        f"/v1/incidents/{incidents[0]['id']}", headers=browser_headers
    ).json()
    assert incident_detail["ticket"]["status"] == "resolved"
    ...
    assert reopened.json()["incident"]["occurrence_count"] == 3
    ...
    action_detail = http.get(
        f"/v1/actions/{action.json()['id']}", headers=browser_headers
    ).json()
    assert action_detail["execution_results"][0]["verified"] is True
    assert action_detail["approvals"][0]["decision"] == "approve"
    assert action_detail["timeline"]
    ticket_detail = http.get(
        f"/v1/tickets/{ticket.json()['id']}", headers=browser_headers
    ).json()
    assert ticket_detail["actions"][0]["id"] == action.json()["id"]
    ...
    deliveries = http.get(
        "/v1/integrations/webhooks/deliveries", headers=browser_headers
    )
    assert deliveries.status_code == 200
    assert deliveries.json()
    disabled = http.delete(
        f"/v1/integrations/webhooks/{webhook.json()['id']}",
        headers=browser_headers,
    )
    assert disabled.status_code == 204
    assert (
        http.get("/v1/integrations/webhooks", headers=browser_headers).json()[0][
            "active"
        ]
        is False
    )
    ...
    other = http.post(
        "/v1/tenants",
        headers={"X-Bootstrap-Token": "test-bootstrap-token"},
        json={"name": "Other", "admin_email": "owner@other.test"},
    ).json()
    assert (
        http.get(
            f"/v1/devices/{device['device_id']}",
            headers={
                "X-Tenant-ID": other["tenant_id"],
                "X-User-ID": other["admin_user_id"],
            },
        ).status_code
        == 404
    )
